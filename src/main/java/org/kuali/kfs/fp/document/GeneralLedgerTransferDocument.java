...
public class GeneralLedgerTransferDocument extends CapitalAccountingLinesDocumentBase implements AmountTotaling,
    ...
    private static final Logger LOG = LogManager.getLogger(GeneralLedgerTransferDocument.class);
    ...
    public boolean answerSplitNodeQuestion(String nodeName) throws UnsupportedOperationException {
        if (nodeName.equals(FULL_ROUTING_SPLIT)) {
            ...
            boolean newAccountFlag = getGeneralLedgerTransferService().newAccountOnTarget(getSourceAccountingLines(), getTargetAccountingLines());
            boolean newObjectCdFlag = getGeneralLedgerTransferService().newObjectCodeOnTarget(getSourceAccountingLines(), getTargetAccountingLines());
            boolean costSharePresentFlag = getGeneralLedgerTransferService().costSharePresent(getSourceAccountingLines(), getTargetAccountingLines());
            LOG.info("answerSplitNodeQuestion:: nodeName=" +nodeName+ "=");
            LOG.info("answerSplitNodeQuestion:: newAccountFlag=" +newAccountFlag+ "=");
            LOG.info("answerSplitNodeQuestion:: newObjectCdFlag=" +newObjectCdFlag+ "=");
            LOG.info("answerSplitNodeQuestion:: costSharePresentFlag=" +costSharePresentFlag+ "=");
            LOG.info("answerSplitNodeQuestion:: BOOLEAN VALUE BEING RETURNED=" + (newAccountFlag || newObjectCdFlag || costSharePresentFlag) + "=");
    ...
    public void customizeExplicitGeneralLedgerPendingEntry(GeneralLedgerPendingEntrySourceDetail postable,
        ...
        handleFiscalPeriodSelection(explicitEntry);
    }
    ...
    private void handleFiscalPeriodSelection(GeneralLedgerPendingEntry explicitEntry) {
        if (StringUtils.isNotBlank(postingPeriodCode)) {
            explicitEntry.setUniversityFiscalPeriodCode(postingPeriodCode);
        }
    }
